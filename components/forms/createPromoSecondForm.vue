<template>
  <div>
    <div v-for="(item, index) in promo2" :key="index">
        ...
        >
          <v-row align="center" justify-sm="space-between">
            ...
            <v-col cols="12" sm="auto">
              <v-btn text :disabled="promo2.length === 1 && !index" color="primary" @click.prevent="delete_promo_row(item)">
...
export default {
  ...
  computed: {
    ...mapMultiRowFields('form', ['promo2'])
  },
  methods: {
    on_submit () {
      _.each(this.promo2, (obj, index) => {
      ...
      setTimeout(() => {
        if (!this.isError) {
          this.$store.dispatch('form/apiCreatePromo', 'second')
        }
      }, 3000)
    },
    on_cancel () {
      ...
      this.$store.commit('form/resetPromo2')
    },
    add_promo_row () {
      this.$store.commit('form/addPromo2Row')
    },
    delete_promo_row (args) {
      this.$store.commit('form/deletePromo2Row', args)
    }
  }
}
</script>
